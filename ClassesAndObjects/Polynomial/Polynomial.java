...
public class Polynomial {
	...
	//Here you goal is add two object of polynomial and Store them into new Object pol3
	 public  static Polynomial addTwo(Polynomial pol, Polynomial pol2) {
	        int maxDegree = Math.max(pol.polynomial.length, pol2.polynomial.length);
	        Polynomial p3 = new Polynomial();

	        for (int i = 0; i < maxDegree; i++) {
	            int polCoefficient = (i < pol.polynomial.length) ? pol.polynomial[i] : 0;
	            int pol2Coefficient = (i < pol2.polynomial.length) ? pol2.polynomial[i] : 0;

	            p3.setCoffecient(i, polCoefficient + pol2Coefficient);
	        }
